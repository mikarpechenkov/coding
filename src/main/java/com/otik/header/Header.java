...
public class Header {
    ...
    public Header() {
        ...
        version = new byte[8];
        for (int i = 0; i != "1.1".getBytes().length; i++) //Изменил версию
            version[i] = "1.1".getBytes()[i];
        for (int i = "1.1".getBytes().length; i != 8; i++)
            version[i] = 0x0;
        sizeOfOriginalFile = new byte[8];
        setSizeOfOriginalFile(0);
        nameOfOriginalFile = new byte[32];
        setNameOfOriginalFile("");
        compressionWithContext = new byte[]{0x1}; //Изменил значение сжатия с контекстом
        compressionWithoutContext = new byte[]{0x0};
        interferenceProtection = new byte[]{0x0};
        codeTable=new byte[64];
        for(byte i:codeTable) i=0x0;
        reserve = new byte[6];
        for (int i = 0; i != 6; i++)
            reserve[i] = 0x0;
    }
